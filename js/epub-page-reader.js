...
window.EpubPageReader = class {
  ...
  makeTurnPage(index) {
    ...
    paper.insertAdjacentHTML("beforeend", `<span class="epub-page-number" aria-hidden="true">${index + 1}</span><span class="epub-corner-cue" aria-hidden="true"></span>`);
    return paper;
  }
  ...
  render() {
    ...
    windowEl.appendChild(columns);
    paper.appendChild(windowEl);
    paper.insertAdjacentHTML("beforeend", '<span class="epub-page-number" aria-hidden="true"></span><span class="epub-corner-cue" aria-hidden="true"></span>');
